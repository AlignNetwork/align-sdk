...
describe("test ipfs", () => {
    ...
    it("should validate failure", async () => {
        ...
        const defaultFormat = AlignDefaults["post"];
        // validte the incoming data is in the correct format
        expect(() => validate(data, defaultFormat)).to.throw(ValidationError, "Missing keys: dateOf");
    });
    it("should validate failure", async () => {
        const data = {
            from: "1",
        ...
        const defaultFormat = AlignDefaults["post"];
        // validte the incoming data is in the correct format
        expect(() => validate(data, defaultFormat)).to.throw(ValidationError, "Extra keys: extraKey");
    });
});
